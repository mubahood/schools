...
class ServiceController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        // Stock Items to be Offered
        $grid->column('items_to_be_offered', 'Stock Items')
            ->display(function ($items) {
                if ($this->to_be_managed_by_inventory !== 'Yes' || empty($items)) {
                ...
                return '<span class="label label-info" style="margin: 2px;">' 
                    . implode('</span> <span class="label label-info" style="margin: 2px;">', $itemNames) 
                    . '</span>';
            });
    ...
    protected function form()
    {
        ...
        $form->radio('to_be_managed_by_inventory', __('Manage by Inventory'))
            ->options([
                'Yes' => 'Yes',
                'No' => 'No',
            ])
            ->default('No')
            ->help('Enable if this service requires stock items to be provided')
            ->when('Yes', function ($form) use ($u) {
                // Multi-select for stock item categories
                $stockCategories = StockItemCategory::where('enterprise_id', $u->enterprise_id)
                    ->orderBy('name', 'asc')
                    ->pluck('name', 'id');
                
                $form->listbox('items_to_be_offered', __('Stock Items to be Offered'))
