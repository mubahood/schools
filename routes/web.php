...
/* Route::get('/', function () {
  ...
  foreach ($books as $key => $n) {
    $t = trim($n->title);
    $current = Book::where(['title' => $t])->first();
    if ($current != null) {
      continue;
    }
    $new = new Book();
    $new->enterprise_id = 1;
    $new->books_category_id = rand(1,10);
    $new->book_author_id = rand(1,10);
    $new->api_id = rand(1,10);
    $new->title = $n->title;
    $new->subtitle = $n->title;
    $new->description = $n->title;
    $new->isbn = rand(1000,100000).rand(1000,1000000);
    $new->page_count = $n->pages;
    $new->price = $n->pages;
    $new->thumbnail = $n->imageLink;
    $new->pdf = $n->imageLink;
    $new->language = $n->language;
    $new->published_date = $n->year;
    $new->quantity = rand(100,200);
