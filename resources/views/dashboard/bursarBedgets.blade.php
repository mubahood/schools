        ...
        <div class="row">
            <div class="col-md-5 pt-2">
                @foreach ($accounts as $class)
                    <a target="_blank" href="{{ admin_url('accounts?account_parent_id=' . $class->id) }}"
                        title="Click to See More Details">
                        <p class="d-flex justify-content-between fc-gray fs-18 fw-600 pb-3  "
                            style="line-height: 1;
                border-bottom: dashed 1px black;">
                            <span>{{ $class->name }}</span>
                            <span class="text-dark">UGX {{ number_format($class->total) }}</span>
                        </p>
                    </a>
                @endforeach
            ...
            <div class="col-md-7">
                <canvas id="bursarBedgets" style="width: 100%;"></canvas>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(function() {
        ...
        var ctx = document.getElementById('bursarBedgets').getContext('2d');
