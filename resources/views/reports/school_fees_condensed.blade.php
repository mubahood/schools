...
<head>
    ...
    <style>
        ...
        /* Header */
        .report-header .logo { width: 100px; }
        ...
        .kpi-card { background-color: #f9f9f9; border: 1px solid #e0e0e0; padding: 8px; border-radius: 4px; height: 65px; }
        ...
        .data-table thead th {
            background-color: {{ $ent->color ?? '#005b96' }};
            color: #fff;
            font-weight: bold;
            text-align: center;
            font-size: 9px;
...
<body>
    ...
    <table class="data-table">
        ...
        <tbody>
            @foreach ($classBreakdown as $class)
                @php
                    $total_students += (int)str_replace(',', '', $class['students']);
                    $total_billed += (int)str_replace(',', '', $class['billed']);
                    $total_collected += (int)str_replace(',', '', $class['collected']);
                    $total_outstanding += (int)str_replace(',', '', $class['outstanding']);
                    $total_fees_bill_per_student += (int)str_replace(',', '', $class['fees_bill_per_student']);
                @endphp
                <tr>
    ...
    @foreach ($studentsByClass as $classId => $studentList)
        @php
            $firstStudent = $studentList->first();
        @endphp
        <div class="student-section-title">{{ $firstStudent['class_name'] }}</div>
        <table class="data-table">
            <thead>
                <tr>
                    <th style="width: 5%;">#</th>
